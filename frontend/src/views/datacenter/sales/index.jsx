...
import { getSalesData } from '../../../api/datacenter/statistics';
...
export default function SalesDataPages() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const result = await getSalesData(selectedMonth);
        if (result.success) {
          setData(result.data);
          setMetrics({
    ...
    if (chartInstances.current.chart3) {
      ...
      chartInstances.current.chart3.on('click', function(event) {
        ...
        if (salespersonId === userInfo.id) {
          ...
          navigate(`/sales/readonly/${salespersonId}/${salespersonName}`);
        }
      });
    }
        ...
        <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div className="flex-1">
              ...
              <p className="text-gray-500 text-lg">实时销售数据统计与分析</p>
            </div>
            ...
            <div className="flex items-center gap-3 bg-gray-50 px-4 py-3 rounded-xl border border-gray-200">
              <div className="flex items-center gap-2">
                ...
                <label className="text-sm font-medium text-gray-600">查看月份：</label>
              </div>
              <select
                value={selectedMonth}
                onChange={(e) => setSelectedMonth(e.target.value)}
                className="bg-white text-gray-700 text-sm font-medium border border-gray-200 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 cursor-pointer rounded-lg px-3 py-1"
              >
                {monthOptions.map((month, index) => (
                  <option key={index} value={month} className="text-gray-700">
                    {month}
                  </option>
                ))}
              </select>
