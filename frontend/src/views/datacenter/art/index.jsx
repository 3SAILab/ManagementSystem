...
import BarChart from './components/BarChart'; // 引入页面专用组件
import { useEmployeePermissionStore } from '../../../store/employee'; // 引入员工权限存储
// import { toast } from 'react-toastify'; // 引入 toast 组件

import { getArtMonthlyCoefficientStatistics, getArtMonthlyAverageCompletionTimeStatistics, getRenderMonthlyCoefficientStatistics, getRenderMonthlyAverageCompletionTimeStatistics } from '../../../api/datacenter/statistics';
...
const ArtDataPage = () => {
  ...
  const handleBarClick = useCallback((params, type) => {
    ...
    navigate('/team/dashboard', { state: { focusName: name } });
