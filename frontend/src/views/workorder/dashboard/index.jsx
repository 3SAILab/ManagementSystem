...
import { getPersonalTasks } from '../../../api/workorder/subtask';
...
const DashboardPage = () => {
  ...
  useEffect(() => {
    getPersonalTasks().then((res) => {
      if(res && res.success && res.data){
        ...
        setYellowAlerts(res.data.yellow_count || 0);
        setRedAlerts(res.data.red_count || 0);
      }else{
  ...
  // 工单分类
  //未开始
  const unStartedOrders = Array.isArray(orders) ? orders.filter((o) => o.status === '未开始') : [];
  //进行中
  const inProgressOrders = Array.isArray(orders) ? orders.filter((o) => o.status === '进行中') : [];
  //已完成
  const completedOrders = Array.isArray(orders) ? orders.filter((o) => o.status === '已完成') : [];
